    ...
    var HarView = function (element, options) {
        ...
        this.timings = function (id, timings) {
            ...
            if(data) {
                ...
                $(element).find('#' + id + '-timeline').append($(html));
                $(element).find('#' + id + '-timeline').attr('title', JSON.stringify(data.timings));
        ...
        var _render = function (id) {
            ...
            $(html).insertBefore($(element).find('#summary'));
            ...
            source = $(element).find('#' + id);
            source.click(function (event) {
                if($(element).find('#' + event.target.id).hasClass('plus')) {
                    $(element).find('#' + event.target.id).removeClass('plus');
                    $(element).find('#' + event.target.id).addClass('minus');
                    $(element).find('#' + event.target.id + '-details').show();
                }
                else {
                    $(element).find('#' + event.target.id).removeClass('minus');
                    $(element).find('#' + event.target.id).addClass('plus');
                    $(element).find('#' + event.target.id + '-details').hide();
                }
            });
            $(element).find('#' + id + '-details').hide();

            // Enable tabbed view
            $(element).find('#' + id + '-tabs').tabs();
        ...
        var _updateRequest = function (id, request) {
            ...
            $(element).find('#' + id + '-url').resizable({handles: 'e'});
            $(element).find('#' + id + '-url').bind('resize', function (event, ui) {
                $(element).find('.url').width(ui.size.width);
            });
            ...
            if(request.queryString && request.queryString.length > 0) {
                ...
                $(element).find('#' + id + '-tabs').tabs('disable', 1);
            }
            if(request.postData && request.postData.text) {
                _updateField('#' + id + '-req-body', request.postData.text);
            }
            else {
                $(element).find('#' + id + '-tabs').tabs('disable', 2);
            }
        };
        ...
        var _updateResponse = function (id, response) {
            ...
            if(response.content && response.content.text) {
                ...
                $(element).find('#' + id + '-tabs').tabs('disable', 3);
            }
        }	

        var _updateField = function (id, field) {
            if(field) {
                $(element).find(id).text(field);
            }
        }
        ...
        var _updateHeaders = function (id, isRequest, headers) {
            ...
            $(element).find('#' + id + (isRequest ? '-req-headers' : '-resp-headers')).append($(html));
        }
        ...
        var _updateQueryString = function (id, queryString) {
            ...
            $(element).find('#' + id + '-query-string').append($(html));
        }
            ...
            $.each(log.entries, function (id, data) {
                if(data.timings) {
                    ...
                    $.each(data.timings, function (key, value) {
                        ...
                        if(width > 0) {
                            $(element).find('#' + id + '-' + key).width(width * frac + '%');
                        }
                        else {
                            $(element).find('#' + id + '-' + key).css('border', 'none');
                        }
                    });
                    $(element).find('#' + id + '-lpad').width(pads[id][0] * frac + '%');
                    $(element).find('#' + id + '-rpad').width(pads[id][1] * frac + '%');
                }
            });
        }
    };
